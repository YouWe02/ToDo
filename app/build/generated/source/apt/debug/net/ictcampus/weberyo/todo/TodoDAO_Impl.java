...
public class TodoDAO_Impl implements TodoDAO {
  ...
  @Override
  public List<Todo> getTodoByWeek(String date) {
    final String _sql = "SELECT * FROM todo WHERE STRFTIME('%j', date(date, '-3 days', 'weekday 4')) - 1 / 7 + 1 = STRFTIME('%j', date(?, '-3 days', 'weekday 4')) - 1 / 7 + 1";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    if (date == null) {
      _statement.bindNull(_argIndex);
  ...
  @Override
  public List<Todo> getTodoByMonth(String date) {
    final String _sql = "SELECT * FROM todo WHERE STRFTIME('%m', date) = STRFTIME('%m', ?)";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    if (date == null) {
      _statement.bindNull(_argIndex);
